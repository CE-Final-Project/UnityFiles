...
{
    public class AttackWeakAIState : AIState
    {
        ...
        public AttackWeakAIState(AIBrain brain, ServerActionPlayer serverActionPlayer)
        {
            m_Brain = brain;
        ...
        private ServerCharacter ChooseFoe()
        {
            ...
            foreach (var foe in m_Brain.GetHatedEnemies())
            {
                ...
                if(foe.HitPoints < 500 && distanceSqr <= 1)
                {
                    closestFoe = foe;  
                }
            }
