...
namespace Script.Lobby
{
    ...
    public class LobbyServiceFacade : IDisposable, IStartable
    {
        ...
        public Unity.Services.Lobbies.Models.Lobby CurrentUnityLobby { get; private set; }
        ...
        public void SetRemoteLobby(Unity.Services.Lobbies.Models.Lobby lobby)
        {
        ...
        /// </summary>
        public async Task<(bool Success, Unity.Services.Lobbies.Models.Lobby Lobby)> TryCreateLobbyAsync(string lobbyName, int maxPlayers, bool isPrivate)
        {
        ...
        /// </summary>
        public async Task<(bool Success, Unity.Services.Lobbies.Models.Lobby Lobby)> TryJoinLobbyAsync(string lobbyId, string lobbyCode)
        {
        ...
        /// </summary>
        public async Task<(bool Success, Unity.Services.Lobbies.Models.Lobby Lobby)> TryQuickJoinLobbyAsync()
        {
        ...
        public async Task<Unity.Services.Lobbies.Models.Lobby> ReconnectToLobbyAsync(string lobbyId)
        {
            try
            {
        ...
        public async Task LeaveLobbyAsync(string lobbyId)
        {
            ...
            try
            {
                await LobbyAPIInterface.RemovePlayerFromLobby(uasId, lobbyId);
            }
            catch (LobbyServiceException e)
            {
                // If Lobby is not found and if we are not the host, it has already been deleted. No need to publish the error here.
        ...
        public async Task DeleteLobbyAsync(string lobbyId)
        {
            if (m_LocalUser.IsHost)
            {
                try
                {
                    await LobbyAPIInterface.DeleteLobby(lobbyId);
                }
                catch (LobbyServiceException e)
                {
                    PublishError(e);
        ...
        public void DoLobbyHeartbeat(float dt)
        {
            ...
            if (m_HeartbeatTime > k_HeartbeatPeriod)
            {
                ...
                try
                {
                    LobbyAPIInterface.SendHeartbeatPing(CurrentUnityLobby.Id);
                }
                catch (LobbyServiceException e)
                {
                    // If Lobby is not found and if we are not the host, it has already been deleted. No need to publish the error here.
